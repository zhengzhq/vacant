...
(function(){
	var modules = {
		block : {
			js : '../../blockUI-2.66/jquery.blockUI.2.66.js'
		},
		xheditor : {
			js : '../../xheditor-1.1.14/xheditor-1.1.14-zh-cn.min.js'
		},
		zTree : {
			js : '../../ztree-3.5.14/jquery.ztree.all-3.5.min.js',
			css : '../../../ztree-3.5.14/css/zTreeStyle.css'
		},
		funcTree : {
			js : '../../../extras/sysFuncTree.js',
			dependencies : ['zTree']
		},
		deptTree : {
			js : '../../../extras/deptTree.js',
			dependencies : ['zTree']
		},
		areaTree : {
			js : '../../../extras/areaTree.js',
			dependencies : ['zTree']
		},
		tabs : {
			js : '../../../extras/jquery.tabs.js'
		},
		table : {
			js : '../../../extras/jquery.table.js'
		},
		draggable:{
...
(function($, undefined){
	if($){
		var modules = easyloader.modules;
		
		var extra = {}, excludes = {messager:{}, parser:''};
		
		var proxyFunc = function(key){
			return function(){
				var me = this, args = arguments;
				if(window.console){
					console.log('Proxy method ' + key + '.');
				}
				easyloader.load(key, function(){
					if(window.console){
						console.log('Proxy method ' + key + ' loaded.');
					}
					var proxy = $.fn[key];
					if(proxy === extra[key]){
						proxy = $[key];
					...
					if(proxy !== undefined)
						...
						console.error('Proxy method ' + key + ' load failed!');
				});
				return me;
			};
		};
		for(var key in modules){
			...
			extra[key] = proxyFunc(key);
			extra[key].defaults = {};
		}
		$.extend($.fn, extra);
	}
	$.ajaxSettings.error = function(){alert('服务器通讯错误!');};
})(window.jQuery);